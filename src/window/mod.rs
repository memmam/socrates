...
//! of the native-OpenGL roadmap (`std.glm`, v0.9, already shipped the math
//! side). Scope for now: window creation, event polling, keyboard/mouse
//! state, and a trivial clear-color + swap-buffers, enough to prove the
//! whole pipe end-to-end. A general `gl` draw-call namespace (a GL
//! function-pointer loader with the usual few dozen entries) is a later PR.
//!
...
//! linked normally, GL/GLX resolved with `dlopen`/`dlsym` at runtime) — so
//! `cargo tree -e normal` stays a single line with or without it.
//!
//! Backend scope for this PR: **Linux/X11/GLX only**. Windows (Win32/WGL)
//! and macOS (Cocoa/NSOpenGL) backends are separate follow-up PRs; on any
//! other target (or with the feature off), [`create`] degrades gracefully
//! to `Err`, the same way `gpu::run` does without the `gpu` feature.
//!
...
//! (release-if-current, destroy context, destroy window, free colormap,
//! close display), so a program that opens/closes many windows in a loop
//! actually reclaims them as they're collected, not only at process exit.
//! Idempotency is via an `Option`-wrapped inner state — `close()`/`Drop`
//! both `take()` it, so a second teardown is a no-op (the same shape as
//! `WorkerHandle::join`'s cached-result idempotency).
...
/// current. Always an `Err` without the `gl` feature or off Linux.
#[cfg(not(all(feature = "gl", target_os = "linux")))]
...
pub fn create(title: &str, w: i32, h: i32) -> Result<WindowHandle, String> {
    let inner = x11::Inner::create(title, w, h)?;
    Ok(WindowHandle { inner: Some(inner) })
}

#[cfg(all(feature = "gl", target_os = "linux"))]
impl WindowHandle {
    /// Pump the X event queue; updates should-close/key/mouse/size state.
    ...
    /// Once `true` (the window manager's close button, `Alt+F4`, etc. was
    /// caught via `WM_DELETE_WINDOW`) it stays `true`. Also `true` once the
    /// window has been explicitly `close()`d.
...
impl WindowHandle {
    pub fn poll(&mut self) {
        unreachable!("WindowHandle is never constructed without --features gl on linux")
    }
    pub fn should_close(&self) -> bool {
        unreachable!("WindowHandle is never constructed without --features gl on linux")
    }
