...
The `socrates` binary is more than a runner. The same checker that types your
...
Any `.soc` file with `//?` directives in its comments is a test. `//?
...
failure, so it drops into CI unchanged — Socrates's own spec suite (311 tests)
...
```sh
socrates test --bless src/          # rewrite mismatched //? expect: lines
```
...
```sh
socrates fmt src/main.soc            # print the formatted source
socrates fmt -w src/*.soc            # rewrite files in place
socrates fmt --width 60 src/main.soc # wrap at 60 columns instead of 100
```
...
## `socrates build` — one program, one binary

`socrates build` turns a whole program into a single self-contained executable.
Point it at a directory whose entry is `main.soc` (or at a `.soc` file
directly):

```sh
socrates build demos/png -o png     # staple the program into ./png
./png                            # run it — no socrates, no source tree needed
```

The binary carries everything the program touches: its imported modules, the
data files it reads, and the separate `.soc` files it hands to
...
promise survives: a stapled binary is just `socrates` with a payload glued on.

The contract is simple to hold in your head: **a stapled binary behaves
exactly like `socrates <the path you built>` run from the directory you built
...
prints precisely what `socrates demos/png/main.soc` prints.
...
socrates build demos/png --launcher ./socrates-aarch64-macos -o png-macos
```
...
linked in as a `__DATA,__socrateszoo` section instead, which `socrates` reads back
out of its own image at startup.

## `socrates lsp`

The checker already knows every type and every definition site; `socrates lsp`
lets your editor ask. It speaks the Language Server Protocol over stdin and
stdout:

- **Diagnostics** as you type — the same checker as `socrates check`, run over
...
scope = "source.soc"
file-types = ["socrates"]
language-servers = ["socrates-lsp"]

[language-server.soc-lsp]
command = "socrates"
args = ["lsp"]
```
...
## `socrates dis` and the rest

`socrates dis file.soc` prints the compiled bytecode — the subject of the
previous chapter, and a good way to see what a piece of syntax actually
costs. `socrates check` type-checks without running; `socrates tokens` and `socrates
ast` dump the earlier pipeline stages for debugging; and `socrates repl` is an
incremental session with working imports and a `:type` query. The full CLI
is `socrates` with no arguments, or `socrates --help`.
