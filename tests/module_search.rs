...
//! the provided search directories (`FABLE_PATH` in the CLI).
...
fn import_resolves_via_search_path() {
    ...
    match fable::run_capture_path_with(&fixture("app/main.fable"), &search) {
...
fn missing_module_reports_all_locations_tried() {
    let search = vec![fixture("shared_lib")];
    match fable::run_capture_path_with(&fixture("app/broken.fable"), &search) {
        RunOutcome::CompileError(diags) => {
            let d = diags.iter().find(|d| d.code == "E0337").expect("an E0337 diagnostic");
            assert!(d.message.contains("no_such_thing"), "message: {}", d.message);
            // The FABLE_PATH candidate appears as a note.
            assert!(
                d.notes.iter().any(|n| n.contains("shared_lib") && n.contains("FABLE_PATH")),
                "notes: {:?}",
                d.notes
            );
        }
...
fn file_relative_wins_over_search_path() {
    // app/ contains its own textutil.fable? No — so add a sibling with the
    // same name in a different fixture to prove precedence.
    let search = vec![fixture("shared_lib")];
    match fable::run_capture_path_with(&fixture("app_shadow/main.fable"), &search) {
        RunOutcome::Ok { stdout, .. } => {
